...
description: >-
  英语词汇故事学习领域：从 FSRS 学习库选目标词，用已会词白名单（范围词库）限定允许出现的英文，
  按用户风格与章节戏剧结构连载中英混合故事（中文叙事 + 英文嵌词），审计入账后等待用户反馈更新记忆；用户明确确认后才写入新词。
platforms: [windows]
metadata:
  version: "2.0.0"
---
...
1. **范围词汇库（已会词白名单）**：定义故事里允许出现的英文词。名字越长，故事里的英文越密——英文比重靠它随学习进度自然爬升（纯英文是坡顶，可用工具的 `mode: english` 切换）。
...
- `engstory_audit_story`：只读审计故事（目标词命中 + 目标词加粗 + 英文只用目标词/已会词/专名 + 混合字数窗；英文占比作为观测值返回）。
...
模型按"章节戏剧结构"写 300–800 混合字数的章节（混合字数 = 汉字数 + 英文词数；审计硬窗口 300–800）：
- 叙事语言是中文；英文只允许目标词 / 范围词库已会词 / 专名白名单三类，功能词也不例外；
- 目标词必须全部出现且每次加粗（如 **abandon**）；争取每词出现 2 次、落在两个不同语境；
- 嵌词处的中文语境要能猜出词义，不紧跟中文翻译注解；
- 紧扣上一章后果与结尾动作，情节层层推进，篇末留下自然镜头动作；
- 遵守负面约束（不重复介绍天气、不随机加人物、不空泛结尾等）。

### 5. 审计

调用 `engstory_audit_story` 检查目标词命中/加粗、允许英文范围与混合字数窗。失败则重写（最多 2 次），不提交失败稿。返回的 `english_ratio` 是观测值不是闸门——它随范围词库增长而上升。
...
- 工具确定检查：目标词全部出现、目标词每次加粗、英文只落在允许三类（目标词/已会词/专名）、混合字数 300–800、重复提交、`summary`/`next_hook`/`chapter_goal`/`consequence` 是否存在、`storyline` 字段是否成功更新。
- 模型自检（写作前必须确认）：本章目标、阻力、主角选择、后果、是否推进线索、结尾是否有具体钩子、是否因目标词引入无关内容。

提交前必须生成以下结构化字段，禁止留空（一律用中文写——它们会回流进下一章的上下文包）：
